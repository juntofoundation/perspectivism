...
/// Right now the "expression" metadata for this Junto-perspective is stored in another expression language and then referenced here by making it a property on the object. Something about this feels off
/// but I cant figure out any other clean way to have this link-lang know what junto perspective is is supposed to represent

/// Getting links via the predicate is also feeling a little messy. Below I mention having some type that allow us to go from type -> predicate string -> type. That should ensure there are no mistakes made 
/// to predicate "syntax" when calling the getLinks function. This should help make things a little cleaner but still feels hacky.
    ...
    async getLinks(query: LinkQuery): Promise<Expression[]> {
        ...
        if (query.predicate != undefined) {
            if (query.predicate == "users") {
                //@ts-ignore
                let data = await axios.get(this.#url + "/perspectives/" + this.#represents.data.address + "/users")
                .then(function (response) {
                    return response.data
                })
                .catch(function (error) {
                ...
                //@ts-ignore
                let data = await axios.get(this.#url + "/expressions?context" + this.#represents.data.address + "&context_type=" + this.#represents.data.perspective_type 
