...
export default class PhotoFormAuthorAdapter implements GetByAuthorAdapter {
    ...
    async getByAuthor(author: Agent, count: number, page: number): Promise<void | Expression[]> {
        const expressions = await axios.get(this.#url + "users/" + author.did + "/expressions?pagination_position=" + page.toString + "&type=PhotoForm&root_expressions=true&sub_expressions=false")
